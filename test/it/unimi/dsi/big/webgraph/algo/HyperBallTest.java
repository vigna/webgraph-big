...
public class HyperBallTest extends WebGraphTestCase {
	...
	public void testTrivial() throws IOException {
		final ImmutableGraph g = ImmutableGraph.wrap(ArrayListMutableGraph.newCompleteBinaryIntree(10).immutableView());
	...
	public static void assertRelativeError(final double sequentialCurrent, final double current, final double threshold) {
		...
		for(final int log2m: new int[] { 4, 5, 6, 8 }) {
			...
			for(final int size: new int[] { 10, 100, 500 }) {
				...
				for(int attempt = 0; attempt < 10; attempt++) {
					...
					final ImmutableGraph g = ImmutableGraph.wrap(view);
					final ImmutableGraph gt = ImmutableGraph.wrap(it.unimi.dsi.webgraph.Transform.transpose(view));
					final HyperBall hyperBall = new HyperBall(g, attempt % 3 == 0 ? null : gt, log2m, null, 0, 10, 10, attempt % 2 == 0, false, false, null, attempt);
		...
		for(final int log2m: new int[] { 4, 5, 6, 8 }) {
			for(final int size: new int[] { 10, 100, 500 }) {
				for(int attempt = 0; attempt < 10; attempt++) {
					...
					final ImmutableGraph g = ImmutableGraph.wrap(view);
					final ImmutableGraph gt = ImmutableGraph.wrap(it.unimi.dsi.webgraph.Transform.transpose(view));
					final HyperBall hyperBall = new HyperBall(g, attempt % 3 == 0 ? null : gt, log2m, null, 0, 10 * (attempt % 3), 10, attempt % 2 == 0, false, false, null, attempt);
					final SequentialHyperBall sequentialHyperBall = new SequentialHyperBall(g, log2m, null, attempt);
					hyperBall.init();
					sequentialHyperBall.init();
					do {
		...
		for(final int log2m: new int[] { 4, 5, 6 }) {
			...
			for(final int size: new int[] { 100, 500, 1000 }) {
				...
				for(int attempt = 0; attempt < 10; attempt++) {
					...
					final it.unimi.dsi.webgraph.ImmutableGraph view = directedCycle.immutableView();
					final ImmutableGraph g = ImmutableGraph.wrap(view);
					final ImmutableGraph gt = ImmutableGraph.wrap(Transform.transpose(view));
					final HyperBall hyperBall = new HyperBall(g, attempt % 3 == 0 ? null : gt, log2m, null, 0, 10 * (attempt % 3), 10, attempt % 2 == 0, false, false, null, attempt);
					final SequentialHyperBall sequentialHyperBall = new SequentialHyperBall(g, log2m, null, attempt);
					hyperBall.init();
					sequentialHyperBall.init();
	...
	public void testInitClosed() throws IOException {
		final ImmutableGraph g = ImmutableGraph.wrap(ArrayListMutableGraph.newCompleteBinaryIntree(3).immutableView());
			...
			for(final int size: new int[] { 10, 100, 500 }) {
				for(int attempt = 0; attempt < 5; attempt++) {
					...
					for (int i = 0; i < 10; i++) {
						...
						final int dist[] = distancesFrom(view, from);
						long totDist = 0;
						int reachable = 0;
						for (int k = 0; k < n; k++)
						...
						final double computedEcc = get(hyperBall.sumOfDistances, from) / hyperBall.count(from);
			...
			for(final int size: new int[] { 10, 100, 500 }) {
				for(int attempt = 0; attempt < 5; attempt++) {
					...
					for (int i = 0; i < 10; i++) {
						...
						final double computedHarm = n / get(hyperBall.sumOfInverseDistances, from);
		...
		for(final int log2m: new int[] { 15 }) {
			for(final int size: new int[] { 10, 100, 500 }) {
				for(int attempt = 0; attempt < 5; attempt++) {
					...
					final int n = (int)g.numNodes();
					for (int i = 0; i < n; i++) {
						assertEquals(get(hyperBall.sumOfDistances, i), get(hyperBall.discountedCentrality[0], i), 1E-5);
						assertEquals(get(hyperBall.sumOfInverseDistances, i), get(hyperBall.discountedCentrality[1], i), 1E-5);
					}
					hyperBall.close();
				}
			}
