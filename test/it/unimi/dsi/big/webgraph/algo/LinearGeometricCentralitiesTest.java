/*
 * Copyright (C) 2011-2025 Paolo Boldi
 *
...
public class LinearGeometricCentralitiesTest {
	...
	public void testPath() throws InterruptedException {
		final it.unimi.dsi.webgraph.ImmutableGraph small = Transform.transpose(new ArrayListMutableGraph(3, new int[][] {
				{ 0, 1 }, { 1, 2 } }).immutableView());
	...
	public void testErdosRenyi() throws IOException, InterruptedException {
		for (final int size : new int[] { 10, 100 }) {
			for (final double density : new double[] { 0.0001, 0.001, 0.01 }) {
				...
				for (int i = 0; i < size; i++) assertEquals(BigArrays.get(hanf.sumOfInverseDistances, i), BigArrays.get(linear.centrality, i), 1E-3);
				hanf.close();
			}
		}
	}
	...
	public void testErdosRenyiBis() throws IOException, InterruptedException {
		for (final int size : new int[] { 10, 100 }) {
			for (final double density : new double[] { 0.0001, 0.001, 0.01 }) {
				...
				for (int i = 0; i < size; i++) assertEquals(BigArrays.get(linear.reachable, i), BigArrays.get(linear.centrality, i), 1E-3);
				...
				for (int i = 0; i < size; i++) {
					if (centralities.closeness[i] != 0) assertEquals(-1.0 / centralities.closeness[i], BigArrays.get(linear.centrality, i), 1E-3);
					else assertEquals(centralities.closeness[i], 0, 1E-3);
				}
