...
public class BVGraphTest extends WebGraphTestCase {
	...
	public void testLarge() throws IOException {
		...
		for(int i = 0; i < g.numNodes(); i++) {
			...
			final LazyLongIterator asciiSuccessors = nodeIterator.successors(), successors = g.successors(i);
	...
	public void testStats() throws IOException {
		...
		for(int i = 0; i < g.numNodes(); i++) {
			nodeIterator.nextLong();
			final int d = (int)nodeIterator.outdegree();
			final long[][] a = nodeIterator.successorBigArray();
			if (d > 0) {
				for (int j = d - 1; j-- != 0;) bin[Fast.mostSignificantBit(get(a, j + 1) - get(a, j))]++;
				final int msb = Fast.mostSignificantBit(Fast.int2nat(get(a, 0) - i));
				if (msb >= 0) bin[msb]++;
			}
		}
