...
public class UnionImmutableGraphTest extends WebGraphTestCase {
	...
	public void testUnion() throws IllegalArgumentException {
		for (final int n : SIZES) {
			final it.unimi.dsi.webgraph.ImmutableGraph g = ArrayListMutableGraph.newCompleteGraph(n, false).immutableView();
			...
			for (int i = 0; i < n; i++) assertArrayEquals(LazyLongIterators.unwrap(union.successors(i)), LazyLongIterators.unwrap(ImmutableGraph.wrap(g).successors(i)));
		}
	}
}
