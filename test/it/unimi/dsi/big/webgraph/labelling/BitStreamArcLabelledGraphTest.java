...
public class BitStreamArcLabelledGraphTest extends WebGraphTestCase {
	...
	public void testCompression() throws IOException, IllegalArgumentException, SecurityException {
		ImmutableGraph g = ImmutableGraph.wrap(new ErdosRenyiGraph(3000, 0.1, 0, false));
	...
	public void testParallelCompressionSparse() throws IOException, IllegalArgumentException, SecurityException {
		ImmutableGraph g = ImmutableGraph.wrap(new ErdosRenyiGraph(300000, 1E-5, 0, false));
		final File basename = BVGraphTest.storeTempGraph(g);
		g = ImmutableGraph.load(basename.toString());
		final String basenameLabel = createGraphWithGammaLabels(basename, g);
		final BitStreamArcLabelledImmutableGraph bsalg = BitStreamArcLabelledImmutableGraph.load(basenameLabel);
	...
	public void testParallelCompressionDense() throws IOException, IllegalArgumentException, SecurityException {
		ImmutableGraph g = ImmutableGraph.wrap(new ErdosRenyiGraph(3000, 0.5, 0, false));
		final File basename = BVGraphTest.storeTempGraph(g);
		g = ImmutableGraph.load(basename.toString());
		final String basenameLabel = createGraphWithGammaLabels(basename, g);
		final BitStreamArcLabelledImmutableGraph bsalg = BitStreamArcLabelledImmutableGraph.load(basenameLabel);
	...
	public String createGraphWithFixedWidthLabels(final File basename, final ImmutableGraph g, final int width) throws IllegalArgumentException, SecurityException, IOException {
		...
		for(int i = 0; i < n; i++) {
			long bits = 0;
			for (final LongIterator j = LazyLongIterators.eager(g.successors(i)); j.hasNext();) bits += labels.writeInt(i * (int)j.nextLong() + i & LABEL_MASK, width);
	...
	public String createGraphWithGammaLabels(final File basename, final ImmutableGraph g) throws IllegalArgumentException, SecurityException, IOException {
		...
		for(int i = 0; i < n; i++) {
			long bits = 0;
			for (final LongIterator j = LazyLongIterators.eager(g.successors(i)); j.hasNext();) bits += labels.writeGamma((int)((i * j.nextLong() + i) & LABEL_MASK));
			offsets.writeLongGamma(bits);
		}
		labels.close();
		offsets.close();
	...
	public void testLabels(final ArcLabelledImmutableGraph alg, final int width) {
		...
		for(final ArcLabelledNodeIterator nodeIterator = alg.nodeIterator(); nodeIterator.hasNext();) {
			...
			for(int i = 0; i < d; i++) {
				if (get(label, i) instanceof AbstractIntLabel) assertEquals(curr + " -> " + get(succ, i), curr * get(succ, i) + curr & LABEL_MASK & mask, get(label, i).getInt());
				else {
