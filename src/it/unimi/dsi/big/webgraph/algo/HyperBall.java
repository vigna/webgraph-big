...
public class HyperBall extends HyperLogLogCounterArray implements SafelyCloseable {
	 ...
	 * @param seed the random seed passed to {@link HyperLogLogCounterArray#HyperLogLogCounterArray(long, long, int, long)}.
	 */
	public HyperBall(final ImmutableGraph g, final ImmutableGraph gt, final int log2m, final ProgressLogger pl, final int numberOfThreads, final int bufferSize, final int granularity, final boolean external, final boolean doSumOfDistances, final boolean doSumOfInverseDistances, final Int2DoubleFunction[] discountFunction, final long seed) throws IOException {
	...
	private long usedMemory() {
		long bytes = 0;
		for(final long[] a: bits) bytes += a.length * ((long)Long.SIZE / Byte.SIZE);
		if (! external) bytes *= 2;
	...
	public void init(final long seed) {
		...
		if (sumOfDistances != null) fill(sumOfDistances, 0);
		if (sumOfInverseDistances != null) fill(sumOfInverseDistances, 0);
		for (int i = 0; i < discountFunction.length; i++) fill(discountedCentrality[i], 0);

		// The initial value (the iteration for this value does not actually happen).
		neighbourhoodFunction.add(last = numNodes);

		fill(modifiedCounter, true); // Initially, all counters are modified.
		...
		public void run() {
			try {
				...
				for(;;) {
					...
					final long upperLimit = local ? localCheckList.length : numNodes;
					...
					for(;;) {
						...
						for(long i = start; i < end; i++) {
							 ...
							 */
							if (!systolic || local || get(mustBeChecked, node)) {
								long d;
								long[][] successor = null;
								...
								for(long j = d; j-- != 0;) {
									final long s = local || systolic ? successors.nextLong() : get(successor, j);
									/* Neither self-loops nor unmodified counter do influence the computation. */
									if (s != node && get(modifiedCounter, s)) {
								...
								if (counterModified && (systolic || doCentrality)) {
									...
									if (doCentrality) {
										...
										if (delta > 0) { // Force monotonicity
											if (doSumOfDistances) BigArrays.add(sumOfDistances, node, (float)(delta * (iteration + 1)));
											if (doSumOfInverseDistances) BigArrays.add(sumOfInverseDistances, node, (float)(delta / (iteration + 1)));
											for (int j = numberOfDiscountFunctions; j-- != 0;) BigArrays.add(discountedCentrality[j], node, (float)(delta * discountFunction[j].get(iteration + 1)));
										}
									}
								}
								...
								if (counterModified) {
									...
									if (!external) set(modifiedResultCounter, node, true);
									...
									if (systolic) {
										...
										else while ((p = predecessors.nextLong()) != -1) set(nextMustBeChecked, p, true);
									}
								...
								if (external) {
									...
									if (counterModified || get(modifiedCounter, node)) setCounter(t, resultBits[chunk], node);
									else unwritten++;
								}
							}
							else if (! external) {
	...
	public void iterate() throws IOException {
		...
		try {
			...
			if (! external) {
				if (previousWasLocal) for (final long x : localCheckList) set(modifiedResultCounter, x, false);
				else fill(modifiedResultCounter, false);
			}
			...
			if (external) {
				...
				if (!preLocal) fill(modifiedCounter, false);
	...
	public static void main(final String arg[]) throws IOException, JSAPException, IllegalArgumentException, ClassNotFoundException, IllegalAccessException, InvocationTargetException, InstantiationException, NoSuchMethodException {
		...
		if (closenessCentrality) {
			...
			for (long i = 0; i < n; i++) {
				final float d = get(hyperBall.sumOfDistances, i);
		...
		if (nieminenCentrality) {
			...
			for (long i = 0; i < n; i++) {
				final double count = hyperBall.count(i);
				dos.writeFloat((float)(count * count - get(hyperBall.sumOfDistances, i)));
			}
			dos.close();
		}
		if (reachable) {
